...
Therefore, we use a simple function generate the appropriate \code{in\_args} argument
...
These results are based around fairly
subpar `NumPy` code for fitting ridge regression models, as the \code{NumPy} 
implementation loops over the first index dimension in a list comprehension. 
Still, for cases when users are writing arbitrary functions that may not be easily vectorizable,
this package provides a gentle wrapper around `JAX` that allows faster
code execution with fairly minimal setup and Pythonic style.
...
the start. Adding the `NumPy` functionality only took a couple of 
